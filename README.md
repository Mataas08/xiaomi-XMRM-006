...
- `0–100` → battery level in percent  
- `-1` → battery not available  
...
```cpp
remote.getLastBatteryLevel();
```

Returns last cached value:

- `0–100` → last known battery level  
- `-1` → no valid data yet  
...
```cpp
remote.hasBatteryLevel();
```

Returns:

- `true` → battery data available  
- `false` → not available  

---

### setBatteryReadInterval()

```cpp
remote.setBatteryReadInterval(ms);
```

Sets battery refresh interval (milliseconds).
